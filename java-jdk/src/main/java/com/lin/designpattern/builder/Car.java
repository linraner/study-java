 ...
 * @Description: from https://juejin.im/post/5c2088205188251920598d85 提供无参的构造函数，暴露一些公共的方法让用户自己去设置对象属性，这种方法较之第一种似乎增强了灵活度，用户可以根据自己的需要随意去设置属性。但是这种方法自身存在严重的缺点： 因为构造过程被分到了几个调用中，在构造中 JavaBean 可能处于不一致的状态。类无法仅仅通过判断构造器参数的有效性来保证一致性。还有一个严重的弊端是，JavaBeans
 * 模式阻止了把类做成不可变的可能。，这就需要我们付出额外的操作来保证它的线程安全。
 **/
public final class Car {
  ...
  public static final class Builder {
    ...
    public Builder safetyBelt(String safetyBelt) {
      if (safetyBelt == null) {
        throw new NullPointerException("没系安全带，不要开车！");
      }
